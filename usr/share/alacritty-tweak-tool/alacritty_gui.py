...
def _build_vte_panel(label_text):
    ...
    vte.set_font(Pango.FontDescription.from_string(f"{ff} {fs:.1f}"))
    box.append(vte)
    _spawn_in_vte(vte)
...
def _build_themes_tab(window):
    ...
    left_panel.set_size_request(429, -1)
...
def _build_appearance_tab(window):
    ...
    detail_box, _, vte_preview = _build_vte_panel("Font Preview")
    ...
    def _update_vte_font(*_):
        ...
        if _vte_creator is not None:
            _vte_creator.set_font(font_desc)
...
def _build_creator_tab(window, notebook):
    ...
    left_scroll.set_size_request(429, -1)
